...
export const Menu = () => {
  ...
  const loadEntries = useCallback((showLoading = true) => {
    if (showLoading) {
      setList(undefined);
    }
    database
  ...
  // Refresh in background when sidebar opens (if already loaded)
  useEffect(() => {
    if (isOpen && list !== undefined) {
      loadEntries(false); // Don't show loading state
    }
  }, [isOpen, list, loadEntries]);
  ...
  // Removed hover-to-open behavior - sidebar now requires explicit click to open
  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="flex selection-accent flex-col side-menu fixed top-0 w-full md:w-[350px] h-full bg-bolt-elements-background-depth-2 border-r md:rounded-r-3xl border-bolt-elements-borderColor border-opacity-50 z-[1000] shadow-2xl hover:shadow-3xl text-sm backdrop-blur-sm transition-shadow duration-300"
      >
