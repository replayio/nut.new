const SkeletonBox = ({ className = '' }: { className?: string }) => (
  <div className={`bg-bolt-elements-background-depth-3 rounded-lg animate-pulse ${className}`} />
);

export const ChatMessagesSkeleton = () => {
  return (
    <div className="flex flex-col gap-4 p-4" style={{ maxWidth: 'var(--chat-max-width, 37rem)', margin: '0 auto' }}>
      {/* User message bubble - rounded like in screenshot */}
      <div className="flex gap-3 items-start">
        <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gradient-to-br from-green-500/30 to-emerald-500/30 border-2 border-green-500/20 flex items-center justify-center">
          <div className="text-green-300 font-semibold text-sm">M</div>
        </div>
        <div className="flex-1 space-y-2">
          <div className="text-xs text-bolt-elements-textSecondary opacity-70">
            <SkeletonBox className="h-3 w-12 inline-block" />
          </div>
          <div className="bg-bolt-elements-background-depth-2 rounded-2xl p-4 border border-bolt-elements-borderColor border-opacity-50 shadow-sm">
            <SkeletonBox className="h-4 w-full max-w-sm" />
          </div>
        </div>
      </div>

      {/* Assistant message bubble - rounded like in screenshot */}
      <div className="flex gap-3 items-start">
        <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gradient-to-br from-orange-500/30 to-amber-600/30 border-2 border-orange-500/20 flex items-center justify-center overflow-hidden">
          <svg width="20" height="20" viewBox="0 0 29 29" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              d="M14.3333 3.66667V1M14.3333 3.66667C9 3.66667 4.33333 6.33333 3.66667 9C3.34267 10.2933 2.44133 11.6027 1 13C2.74667 12.8907 3.62933 12.6133 5 11.6667M14.3333 3.66667C19.6667 3.66667 24.3333 6.33333 25 9C25.3227 10.2933 25.9373 11.6187 27.6667 13C25.9107 13.2093 25.0427 12.888 23.6667 11.6667M5 11.6667V17C5.00022 19.0758 5.69206 21.0924 6.96623 22.7312C8.2404 24.37 10.0243 25.5375 12.036 26.0493C12.5853 26.188 13.1053 26.4387 13.5053 26.8387L14.3333 27.6667L15.1613 26.8387C15.5613 26.4387 16.0813 26.188 16.6307 26.0493C18.6425 25.5377 20.4265 24.3702 21.7007 22.7314C22.9749 21.0926 23.6667 19.0759 23.6667 17V11.6667M5 11.6667C5.72 12.8933 6.30933 13.4747 7.66667 14.3333C9.60267 13.4707 10.272 12.8693 11 11.6667C11.7933 12.9933 12.5347 13.5693 14.3333 14.3333C16.08 13.5053 16.816 12.9227 17.6667 11.6667C18.5053 12.9693 19.216 13.564 21 14.3333C22.612 13.6027 23.24 13.044 23.6667 11.6667"
              stroke="currentColor"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              className="text-orange-300"
            />
          </svg>
        </div>
        <div className="flex-1 space-y-2">
          <div className="text-xs text-bolt-elements-textSecondary opacity-70">
            <SkeletonBox className="h-3 w-16 inline-block" />
          </div>
          <div className="bg-bolt-elements-background-depth-2 rounded-2xl p-4 border border-bolt-elements-borderColor border-opacity-50 shadow-sm space-y-2">
            <SkeletonBox className="h-4 w-full" />
            <SkeletonBox className="h-4 w-[95%]" />
            <SkeletonBox className="h-4 w-[90%]" />
          </div>
        </div>
      </div>
    </div>
  );
};
