...
export function GlobalDeployChatModal({ variant = 'modal' }: GlobalDeployChatModalProps) {
  ...
  const modalBody = (
    <>
      ...
      <div className={classNames('mt-6 overflow-y-auto flex-1 min-h-0', isSmallViewport ? 'p-4' : 'p-6')}>
        {loadingData ? (
          <div className="text-center">
            ...
            <h3 className={classNames('font-bold text-foreground', isSmallViewport ? 'text-xl mb-2' : 'text-2xl mb-3')}>
              Loading data...
            </h3>
          ...
          <>
            <div className={classNames('text-center', isSmallViewport ? 'mb-4' : 'mb-6')}>
              <h2 className={classNames('font-bold text-foreground', isSmallViewport ? 'text-xl' : 'text-2xl')}>
            ...
            <Tabs
              value={activeTab}
              onValueChange={(v) => deployModalStore.setActiveTab(v as 'default' | 'custom')}
              className="w-full"
            >
              <TabsList
                className={classNames(
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Choose your applications name and deploy it for free
                    </p>
                    <label className="block text-sm font-semibold text-foreground mt-4 mb-2">
                      Your application's URL
                    </label>
                    <div className="flex items-center gap-2 p-3 rounded-lg border border-border bg-muted/30">
                      <code className={classNames('flex-1 text-sm text-muted-foreground font-mono min-w-0 truncate')}>
                        {previewURL}
                      </code>
                      <Button
                        variant="outline"
              ...
              <TabsContent value="custom" className="mt-0">
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-4">
                      Deploy this application to a domain of your choosing
                    </p>
                    ...
                    {deploySettings.siteName && (
                      <div className="mt-2 p-3 bg-muted rounded-md border border-border">
                        <p className="text-sm text-muted-foreground">
                          ...
                          <br />
                          <span
                            className={classNames('font-mono text-primary text-sm block truncate', {
  ...
  if (variant === 'popover') {
    return <div className="relative flex flex-col max-h-[85vh] overflow-hidden">{modalBody}</div>;
  }
