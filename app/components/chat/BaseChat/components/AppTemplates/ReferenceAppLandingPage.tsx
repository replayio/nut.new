...
const StageBadge: React.FC<{ stage: string }> = ({ stage }) => {
  ...
  return (
    <span className={`inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full ring-1 ring-inset ${style}`}>
      {stage}
    </span>
  );
};
      ...
      {artifactURLs && artifactURLs.length > 0 && (
        <div className="mt-4 flex gap-3 flex-wrap">
          {artifactURLs.map((url, urlIndex) => {
            ...
            return isVideo ? (
              <div key={urlIndex} className="relative rounded-lg overflow-hidden border border-bolt-elements-borderColor shadow-sm">
                <video
                  src={url}
                  controls
                  playsInline
    ...
    <div className="p-8">
      <div className="flex flex-col items-center justify-center gap-6 py-16">
        <div className="relative">
          ...
          <div className="absolute inset-0 w-16 h-16 rounded-full border-4 border-transparent border-t-pink-500/30 animate-spin" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }} />
        </div>
        ...
        {(appPreviewURL || isPreviewLoading) && (
            <div>
              ...
              <div className="relative rounded-xl overflow-hidden border border-bolt-elements-borderColor shadow-lg bg-white">
                  ...
                  <div className="absolute inset-0 pt-8 bg-bolt-elements-background-depth-1 flex flex-col items-center justify-center z-[5]">
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <div className="relative">
                        ...
                        {/* Pulsing ring */}
                        <div className="absolute -inset-2 rounded-3xl border-2 border-rose-500/30 animate-ping" style={{ animationDuration: '1.5s' }} />
                      </div>
                      {/* Loading text */}
                      <div className="text-center">
                ...
                {/* Actual Preview */}
                <div className={`pt-8 transition-opacity duration-300 ${isPreviewLoading ? 'opacity-0' : 'opacity-100'}`} style={{ height: '500px' }}>
                  {appPreviewURL && (
                    <AppView
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {displayData.bulletPoints.map((point, index) => (
                  <div key={index} className="flex items-start gap-3 p-3 rounded-lg bg-bolt-elements-background-depth-2/50 border border-bolt-elements-borderColor/50">
