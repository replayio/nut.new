...
import { NutLogo } from './vertical-navbar';
    ...
    return (
      <div className="px-2">
        <form onSubmit={handleSubmit} className="flex items-center gap-2">
          <input
            type="text"
            className="flex-1 bg-background text-foreground rounded-md px-2 py-1.5 text-sm border border-border focus:ring-2 focus:ring-ring focus:outline-none"
            autoFocus
            value={currentTitle}
...
export function Sidebar({ isOpen, onToggle }: SidebarProps) {
  ...
  return (
    <div
      className={`absolute left-2 top-2 bottom-2 z-50 flex flex-col rounded-xl border border-border bg-sidebar shadow-2xl transition-all duration-300 ${
      ...
      <div className="flex flex-col gap-3 p-3 border-b border-border">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <NutLogo />
            <span className="font-semibold text-sidebar-foreground">Nut</span>
          </div>
          {user ? (
            <UserProfileMenu />
          ) : (
      ...
      <ScrollArea className="flex-1 px-2 py-3">
        {list === undefined ? (
          ...
          <div className="flex flex-col items-center justify-center py-8 text-center px-4">
            ...
            <p className="text-sm font-medium text-sidebar-foreground">No apps yet</p>
            <p className="text-xs text-muted-foreground mt-1">Create your first app</p>
          </div>
        ) : (
