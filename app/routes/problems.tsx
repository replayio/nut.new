...
export function Status({ status }: { status: BoltProblemStatus | undefined }) {
  ...
  return (
    <div className="flex items-center gap-2 my-2">
      ...
      <div
        className={`inline-flex items-center px-3 py-1 rounded-full bg-opacity-10 ${statusColors[status]} text-${status}`}
      >
              ...
              <div className="grid gap-4">
                {filteredProblems?.map((problem) => (
                  ...
                  >
                    ...
                    <p className="font-medium">Time: {new Date(problem.timestamp).toLocaleString()}</p>
                  </a>
                ))}
              </div>
            )}
