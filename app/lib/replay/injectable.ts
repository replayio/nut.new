...
function getAllDependencies(dependencies: Record<string, Dependency>, allDependencies: Record<string, AnyFunction> = {}) {
...
function asCallString(dependencies: Record<string, Dependency>, fn: AnyFunction) {
  const dependenciesString = Object.entries(getAllDependencies(dependencies)).map(([key, value]) => `${JSON.stringify(key)}: ${value}`).join(',\n');
  return `(${fn})({\n${dependenciesString}\n})`;
}

function validateDependencies(rootDependencies: Record<string, Dependency>, dependencies: Record<string, Dependency>) {
  for (const [key, value] of Object.entries(dependencies)) {
    assert(!(key in rootDependencies) || rootDependencies[key] === dependencies[key], `"${key}" dependency is not the same as the root dependency at the same key`);
...
export function createInjectableFunction<
  Dependencies extends Record<string, Dependency>,
